...
void	mlx_start(t_infos *infos)
{
	...
	mlx_get_screen_size(infos->mlx.mlx, &infos->mlx.screenx,
		&infos->mlx.screeny);
	infos->resx = (infos->resx < infos->mlx.screenx)
		? infos->resx : infos->mlx.screenx;
	...
	infos->mlx.addr = (int *)mlx_get_data_addr(infos->mlx.img,
		&infos->mlx.bits_per_pixel, &infos->mlx.line_length,
		&infos->mlx.endian);
	if (infos->save == 1)
		raycasting(infos);
